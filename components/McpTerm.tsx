"use client";

import { useEffect, useRef, useState } from "react";

const ROWS = [
  { k: "mood", v: "settling" },
  { k: "themes", v: "work · sleep · dad" },
  { k: "streak", v: "12 days" },
  { k: "energy", v: "recovering" },
  { k: "growth", v: "deciding → replaying" },
];

const AI_RESPONSE =
  "yeah. three rough nights, work piling up, and that conversation with your dad is still sitting with you. want to start there?";

export function McpTerm() {
  const termRef = useRef<HTMLDivElement | null>(null);
  const [shown, setShown] = useState<{
    you: boolean;
    toolCall: boolean;
    rows: number[];
    ai: string;
  }>({
    you: false,
    toolCall: false,
    rows: [],
    ai: "",
  });

  useEffect(() => {
    const el = termRef.current;
    if (!el) return;

    const prefersReduced = window.matchMedia(
      "(prefers-reduced-motion: reduce)"
    ).matches;

    if (prefersReduced) {
      setShown({
        you: true,
        toolCall: true,
        rows: ROWS.map((_, i) => i),
        ai: AI_RESPONSE,
      });
      return;
    }

    const io = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting) return;
          io.disconnect();
          const timers: number[] = [];

          // 1. User message appears
          timers.push(
            window.setTimeout(() => setShown((s) => ({ ...s, you: true })), 300)
          );
          // 2. Tool call block appears
          timers.push(
            window.setTimeout(
              () => setShown((s) => ({ ...s, toolCall: true })),
              900
            )
          );
          // 3. Rows stream in
          ROWS.forEach((_, i) => {
            timers.push(
              window.setTimeout(
                () =>
                  setShown((s) => ({ ...s, rows: [...s.rows, i] })),
                1200 + i * 180
              )
            );
          });
          // 4. Claude response typewriter
          const aiStart = 1200 + ROWS.length * 180 + 400;
          let ci = 0;
          const type = () => {
            if (ci > AI_RESPONSE.length) return;
            setShown((s) => ({ ...s, ai: AI_RESPONSE.slice(0, ci) }));
            ci += 2;
            timers.push(window.setTimeout(type, 22));
          };
          timers.push(window.setTimeout(type, aiStart));
        });
      },
      { threshold: 0.3 }
    );
    io.observe(el);
    return () => io.disconnect();
  }, []);

  return (
    <div className="claude-win" data-in ref={termRef}>
      <div className="claude-win__chrome">
        <span className="claude-win__dots">
          <span className="claude-win__dot claude-win__dot--r" />
          <span className="claude-win__dot claude-win__dot--y" />
          <span className="claude-win__dot claude-win__dot--g" />
        </span>
        <span className="claude-win__url">
          <svg
            width="10"
            height="10"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
            aria-hidden="true"
          >
            <rect x="3" y="11" width="18" height="11" rx="2" />
            <path d="M7 11V7a5 5 0 0 1 10 0v4" />
          </svg>
          claude.ai/chat/mialo
        </span>
        <span className="claude-win__menu">
          <span />
          <span />
          <span />
        </span>
      </div>

      <div className="claude-win__body">
        {/* user message */}
        <div
          className={`claude-row claude-row--user ${shown.you ? "is-on" : ""}`}
        >
          <div className="claude-bubble">can we talk through this week?</div>
        </div>

        {/* claude response block */}
        <div
          className={`claude-row claude-row--ai ${
            shown.toolCall ? "is-on" : ""
          }`}
        >
          <div className="claude-avatar" aria-hidden="true">
            <ClaudeMark />
          </div>
          <div className="claude-ai-body">
            <div className="claude-name">Claude</div>

            {/* tool call block */}
            <div
              className={`claude-tool ${shown.toolCall ? "is-open" : ""}`}
            >
              <div className="claude-tool__head">
                <span className="claude-tool__tag">
                  <svg
                    width="12"
                    height="12"
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    aria-hidden="true"
                  >
                    <path d="M5 12l-2 0" />
                    <path d="M19 12l2 0" />
                    <path d="M12 5l0-2" />
                    <path d="M12 19l0 2" />
                    <circle cx="12" cy="12" r="4" />
                  </svg>
                  Called MCP tool
                </span>
                <span className="claude-tool__src">mialo.read_context</span>
              </div>
              <div className="claude-tool__body">
                {ROWS.map((r, i) => (
                  <div
                    key={r.k}
                    className={`claude-tool__row ${
                      shown.rows.includes(i) ? "is-on" : ""
                    }`}
                  >
                    <span className="claude-tool__k">{r.k}</span>
                    <span className="claude-tool__v">{r.v}</span>
                  </div>
                ))}
              </div>
            </div>

            {/* claude response text */}
            <p className="claude-reply">
              {shown.ai}
              {shown.ai.length > 0 && shown.ai.length < AI_RESPONSE.length && (
                <span className="claude-caret" />
              )}
            </p>
          </div>
        </div>
      </div>

      {/* fake reply input at the bottom */}
      <div className="claude-input">
        <span>Reply to Claude…</span>
        <span className="claude-input__send" aria-hidden="true">
          <svg
            width="14"
            height="14"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2.2"
            strokeLinecap="round"
            strokeLinejoin="round"
          >
            <path d="M5 12h14" />
            <path d="m12 5 7 7-7 7" />
          </svg>
        </span>
      </div>
    </div>
  );
}

function ClaudeMark() {
  // Official Claude logo path (source: claude.com)
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M11.376 24L10.776 23.544L10.44 22.8L10.776 21.312L11.16 19.392L11.472 17.856L11.76 15.96L11.928 15.336L11.904 15.288L11.784 15.312L10.344 17.28L8.16 20.232L6.432 22.056L6.024 22.224L5.304 21.864L5.376 21.192L5.784 20.616L8.16 17.568L9.6 15.672L10.536 14.592L10.512 14.448H10.464L4.128 18.576L3 18.72L2.496 18.264L2.568 17.52L2.808 17.28L4.704 15.96L9.432 13.32L9.504 13.08L9.432 12.96H9.192L8.4 12.912L5.712 12.84L3.384 12.744L1.104 12.624L0.528 12.504L0 11.784L0.048 11.424L0.528 11.112L1.224 11.16L2.736 11.28L5.016 11.424L6.672 11.52L9.12 11.784H9.504L9.552 11.616L9.432 11.52L9.336 11.424L6.96 9.84L4.416 8.16L3.072 7.176L2.352 6.672L1.992 6.216L1.848 5.208L2.496 4.488L3.384 4.56L3.6 4.608L4.488 5.304L6.384 6.768L8.88 8.616L9.24 8.904L9.408 8.808V8.736L9.24 8.472L7.896 6.024L6.456 3.528L5.808 2.496L5.64 1.872C5.576 1.656 5.544 1.416 5.544 1.152L6.288 0.144001L6.696 0L7.704 0.144001L8.112 0.504001L8.736 1.92L9.72 4.152L11.28 7.176L11.736 8.088L11.976 8.904L12.072 9.168H12.24V9.024L12.36 7.296L12.6 5.208L12.84 2.52L12.912 1.752L13.296 0.840001L14.04 0.360001L14.616 0.624001L15.096 1.32L15.024 1.752L14.76 3.6L14.184 6.504L13.824 8.472H14.04L14.28 8.208L15.264 6.912L16.92 4.848L17.64 4.032L18.504 3.12L19.056 2.688H20.088L20.832 3.816L20.496 4.992L19.44 6.336L18.552 7.464L17.28 9.168L16.512 10.536L16.584 10.632H16.752L19.608 10.008L21.168 9.744L22.992 9.432L23.832 9.816L23.928 10.2L23.592 11.016L21.624 11.496L19.32 11.952L15.888 12.768L15.84 12.792L15.888 12.864L17.424 13.008L18.096 13.056H19.728L22.752 13.272L23.544 13.8L24 14.424L23.928 14.928L22.704 15.528L21.072 15.144L17.232 14.232L15.936 13.92H15.744V14.016L16.848 15.096L18.84 16.896L21.36 19.224L21.48 19.8L21.168 20.28L20.832 20.232L18.624 18.552L17.76 17.808L15.84 16.2H15.72V16.368L16.152 17.016L18.504 20.544L18.624 21.624L18.456 21.96L17.832 22.176L17.184 22.056L15.792 20.136L14.376 17.952L13.224 16.008L13.104 16.104L12.408 23.352L12.096 23.712L11.376 24Z" />
    </svg>
  );
}
